...
<?php
    ...
    // query
    // - fetch()[bir tane veri çeker] - fetchAll()
    // prepare-execute
    ...
    <ul>
        <?php foreach($dersler as $ders):?>
            <li>
                ...
                <?php 
                    $kateforiAdlari = explode(',', $ders['kategori_adi']);
                    $kateforiIdlari = explode(',', $ders['kategori_id']);
                    foreach($kateforiAdlari as $key => $value){
                        echo '<a href="index.php?sayfa=kategori&id=' . $kateforiIdlari[$key] . '">' . $value . '</a> ';
                    }
                ?>
                <div>
                    <?php if($ders['onay'] == 1):?>
